...
const NotificationLogs = () => {
  ...
  const handleResend = async (log: NotificationLog) => {
    ...
    try {
      if (log.notification_type === "stock_alert") {
        ...
        const invoiceId = (log.metadata as Record<string, unknown>)
      ...
      toast.error(
        "فشل في إعادة الإرسال: " + (error.message || "خطأ غير معروف")
      );
    } finally {
      setResending(null);
    }
  ...
  const stats = {
    ...
    stockAlerts: logs.filter((l) => l.notification_type === "stock_alert")
      .length,
    invoiceReminders: logs.filter(
      (l) => l.notification_type === "invoice_reminder"
    ).length,
  };
                ...
                <div className="w-48">
                  <Select value={filterType} onValueChange={setFilterType}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="invoice_reminder">
                        تذكيرات الفواتير
                      </SelectItem>
                    </SelectContent>
                  </Select>
                                          ...
                                          {selectedLog.notification_type ===
                                            ...
                                            <>
                                              <p className="text-sm">
                                                ...
                                                {((
                                                  selectedLog.metadata as Record<
                                                    string,
                                                    unknown
                                                  >
                                                )
                                                  ?.out_of_stock_count as number) ||
                                                  0}
                                              </p>
                                              <p className="text-sm">
                                                <span className="text-muted-foreground">
